...
## The wardrobe: what the app does with a stored garment

Saving a garment does more than record its labels. The web app is a wardrobe,
and everything below is built on the same principle as the weather: the model
reports what it sees, and the reasoning on top of that lives in the graph and in
plain tables, never hidden inside the model.

**Only the clothing is kept.** Background removal uses `u2net_cloth_seg`, a
segmentation model trained on clothing rather than on salient foreground. The
general model keeps whoever is wearing the coat, face and hands included; the
cloth model keeps the coat and drops the person. A wardrobe of faceless cutouts
is both what a wardrobe should look like and what the classifier should be shown.
If the cloth weights cannot be downloaded, it falls back to the general model and
says so once. Each cutout is cropped to its own bounding box, so a garment fills
its card instead of floating in a sea of transparent frame.

**Several garments in one photo are found separately.** The same cloth model
segments a picture into upper-body, lower-body and full-body clothing, and
`segment_garments` reads those three masks back: a photo of one jumper returns
one garment, a photo of a whole outfit returns two or three, each cropped to
itself and classified on its own. A one-piece and a two-piece reading are
mutually exclusive, so a dress does not also come back as a top and a skirt. The
`/analyze` response is therefore a list — `{count, garments: [...]}` — and each
garment carries its own correctable handle.

**A brand tag estimates a price.** The upload takes an optional brand. There is
no product feed here, so the price is an honest estimate and labelled one
everywhere: a per-category base price (`CATEGORY_BASE_PRICE`) scaled by the
brand's market tier (`BRAND_TIERS`: H&M budget, Zara mid, Nike premium, Gucci
luxury). An unknown brand is assumed mid-range and said to be a guess. The owner
corrects it in one tap, and their number replaces the estimate and is marked
`price_source: 'you'` rather than `'estimated'`.

**Where to buy one like it.** Each garment offers search links to real fashion
retailers (Google Shopping, Zalando, About You, ASOS, H&M), built from its own
description: a "blue denim jeans" reading becomes a `blue denim jeans` search on
each. The system sells nothing and stores no prices; the shops quote their own
live prices and discounts. Nothing here is invented, which is the only honest way
to answer "where can I get one" without a product feed of our own.

**Your own price and care.** Two facts the model cannot see and the owner can:
what a garment cost (in `CURRENCY`, default EUR), and how it is actually washed.
Both are edited on the card and stored on the `Garment` node. The wash suggestion
is derived from the fibre (`WASH_CARE` in `config.py`: wool 30 on a wool cycle,
polyester 40 but never hot, leather not in a machine at all) and can be
overridden per garment.

**What to wear.** `/recommend?season=` assembles one outfit from what is actually
owned, filling the outfit slot by slot with garments warm enough for the target
and best matching the person's stated taste (`data/preferences.json`, tilted a
notch by "I feel the cold"). It is a greedy per-slot heuristic and is described as
one; when the wardrobe cannot cover the weather it lists what is missing rather
than padding the outfit with something unsuitable.

**What is worth it, and what is missing.** `/wardrobe/profile` reads the whole
collection back to its owner: the styles and colours it leans on, its total
value, the most and least valuable outfit that can be assembled from priced
pieces, and a season-by-season gap analysis. A season is "ready" only when there
is something warm enough for the upper body, the lower body, and, for the cold
seasons, a real outer layer; whatever is missing is the honest shopping list.

**Admin statistics are separate.** The graph-wide counts, the confusion table and
the corpus figures live on their own page (`#/admin` in the app), because they
are the maintainer's view of the whole database, not the owner's view of one
wardrobe. Neither is accuracy; that is still `evaluate.py` against labelled
ground truth, and the admin page repeats the warning so a confident count is
never mistaken for a score.

A conversational layer is deliberately **not** built yet. The seams for it exist
(the graph answers in Cypher, the ontology is served, the routes are small), and
when it comes it will use parameterised query templates, never free-form Cypher
generated by an LLM against a database with a single admin account.
...
pipeline/
  remove_bg.py       cloth-segmentation cutout (keeps clothing, drops the wearer)
  ...
  closet.py          recommender, gap analysis, value, wash care, profile
  prefs.py           one person's stated taste, a small JSON file
frontend/            React + TypeScript + Vite web app
  src/api.ts         the only file that knows where the backend is
  src/shop.ts        real retailer search links from a garment's attributes
